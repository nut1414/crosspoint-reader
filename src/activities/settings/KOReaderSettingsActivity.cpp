...
const StrId menuNames[MENU_ITEMS] = {StrId::STR_USERNAME,        StrId::STR_PASSWORD,
                                     ...
                                     StrId::STR_WIFI_CONNECTION, StrId::STR_AUTHENTICATE};
}  // namespace
...
void KOReaderSettingsActivity::handleSelection() {
  if (selectedIndex == 0) {
    ...
    const uint8_t current = static_cast<uint8_t>(KOREADER_STORE.getWifiConnectionMode());
    const uint8_t count = static_cast<uint8_t>(KOReaderWifiConnectionMode::COUNT);
    KOREADER_STORE.setWifiConnectionMode(normalizeKOReaderWifiConnectionMode((current + 1) % count));
    KOREADER_STORE.saveToFile();
    requestUpdate();
